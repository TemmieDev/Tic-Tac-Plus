...
func _input(event):
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			...
			if event.position.x < board_size and event.position.y < board_size + 240 and event.position.y > 240:
				#convert mouse position into grid location
				grid_pos = Vector2i(event.position / cell_size)
				if grid_data[grid_pos.y - 1][grid_pos.x] == 0:
					moves += 1
					grid_data[grid_pos.y - 1][grid_pos.x] = player
					...
					#check is the board has been filled
					elif moves == 9:
						get_tree().paused = true
						$GameOverMenu.show()
...
func check_win():
	#add up markers in each row. column and diagonal
	for i in len(grid_data):
		...
		#check if either player has all ofthe markers in one line
